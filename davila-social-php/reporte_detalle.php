    ...
    <div class="max-w-5xl mx-auto p-8 space-y-8 w-full">
        ...
        <div class="glass-panel p-8 rounded-3xl relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6 border-violet-500/30">
            <div class="flex items-center gap-5">
                <img src="<?= htmlspecialchars($report['client_logo']) ?>" class="w-16 h-16 rounded-2xl object-cover border border-slate-200 dark:border-dark-border shadow-lg">
                <div>
        ...
        <div class="glass-panel p-6 rounded-2xl space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                <?php foreach ($posts as $p): ?>
                    <div class="bg-slate-50 dark:bg-slate-900/60 rounded-xl overflow-hidden border border-slate-200 dark:border-dark-border">
                        <img src="<?= htmlspecialchars($p['media_url'] ?? $p['thumbnail_url']) ?>" class="w-full h-32 object-cover">
                        <div class="p-3 space-y-2">
                            <p class="text-xs text-slate-700 dark:text-slate-300 line-clamp-2"><?= htmlspecialchars($p['caption']) ?></p>
                            <div class="flex justify-between text-[11px] font-bold text-slate-500 dark:text-slate-400">
                                <span><?= number_format($p['likes']) ?> likes</span>
                                <span class="text-violet-500"><?= $p['engagement_rate'] ?>% ER</span>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
